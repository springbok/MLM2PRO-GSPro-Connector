...
class Device(QObject):
    ...
    def add_device(self, info):
        print(f'info: {info.name()} {info.name().startswith("MLM2-") or info.name().startswith("BlueZ ")}')
        if info.coreConfigurations() & QBluetoothDeviceInfo.LowEnergyCoreConfiguration and info.name() != "" and \
            (info.name().startswith("MLM2-") or info.name().startswith("BlueZ ")):
            self.update = "Found MLM2PRO: " + info.name()

    @Slot()
    def device_scan_finished(self):
        foundDevices = self.discovery_agent.discoveredDevices()
        for nextDevice in foundDevices:
            if nextDevice.coreConfigurations() & QBluetoothDeviceInfo.LowEnergyCoreConfiguration and \
                (nextDevice.name().startswith("MLM2-") or nextDevice.name().startswith("BlueZ ")):
    ...
    def add_low_energy_service(self, service_uuid):
        print(f'add_low_energy_service {service_uuid}')
