...
from util import get_sensor_address, get_sensor_remote_address
...
class SensorScanner(QObject):
    ...
    def _handle_scan_result(self):
        print(f'_handle_scan_result {self.scanner.discoveredDevices()}')
        polar_sensors: list[QBluetoothDeviceInfo] = [
            d
            for d in self.scanner.discoveredDevices()
            if "KICKR" in str(d.name()) and d.rssi() <= 0
        ...
        print(f'Found {len(polar_sensors)} sensor(s).')
