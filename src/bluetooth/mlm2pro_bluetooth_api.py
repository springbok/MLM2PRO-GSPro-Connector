...
class MLM2PROBluetoothAPI:
    ...
    def __init__(self, device: BLEDevice):
        self.mlm2pro_client = BluetoothClient()
        self.general_service = None
        self.notifications = [
            MLM2PROBluetoothAPI.EVENTS_CHARACTERISTIC_UUID,
            MLM2PROBluetoothAPI.HEARTBEAT_CHARACTERISTIC_UUID,
            MLM2PROBluetoothAPI.WRITE_RESPONSE_CHARACTERISTIC_UUID,
            MLM2PROBluetoothAPI.MEASUREMENT_CHARACTERISTIC_UUID
        ]
        self.device = device
        self.armed = False
        self.started = False
    ...
    async def start(self):
        ...
        print('api start')
        self.general_service = self.mlm2pro_client.bleak_client.services.get_service(MLM2PROBluetoothAPI.SERVICE_UUID)
    ...
    async def read_firmware_version(self):
        ...
        characteristic = self.general_service.get_characteristic(MLM2PROBluetoothAPI.firmware_characteristic_uuid)
    ...
    def notification_handler(self, characteristic: BleakGATTCharacteristic, data: bytearray):
        ...
        if characteristic.uuid.upper() == MLM2PROBluetoothAPI.WRITE_RESPONSE_CHARACTERISTIC_UUID:
        ...
        elif characteristic.uuid.upper() == MLM2PROBluetoothAPI.HEARTBEAT_CHARACTERISTIC_UUID:
    ...
    def process_write_response(self, data: list[int]):
        if len(data) >= 2:
            if len(data) > 2:
                if data[0] == MLM2PROBluetoothAPI.MLM2PRO_SEND_INITIAL_PARAMS:
                    print(f'Auth requested: Initial parameters need to be sent to MLM2PRO {data[0]}')
                    if data[1] != MLM2PROBluetoothAPI.MLM2PRO_AUTH_SUCCESS or len(data) < 4:
                        print(f'Auth failed: {data[1]}')
                        if data[1] == MLM2PROBluetoothAPI.MLM2PRO_RAPSODO_AUTH_FAILED:
                    ...
                    asyncio.create_task(self.send_initial_params(data))
            else:
        ...
        while self:
            ...
            if self.mlm2pro_client.is_connected:
                ...
                if datetime.datetime.utcnow() > self.next_heartbeat:
                    ...
                    self.set_next_expected_heartbeat()
                    await self.subscribe_to_characteristics()
                await self.mlm2pro_client.write_characteristic(self.general_service, bytearray([0x01]), MLM2PROBluetoothAPI.HEARTBEAT_CHARACTERISTIC_UUID)
            await asyncio.sleep(MLM2PROBluetoothAPI.HEARTBEAT_INTERVAL)
    ...
    def set_next_expected_heartbeat(self):
        ...
        self.next_heartbeat = now + datetime.timedelta(seconds=MLM2PROBluetoothAPI.MLM2PRO_HEARTBEAT_INTERVAL)
    ...
    async def auth(self):
        ...
        await self.mlm2pro_client.write_characteristic(self.general_service,
            b_arr,
            MLM2PROBluetoothAPI.AUTH_CHARACTERISTIC_UUID, True)
    ...
    async def write_config(self, data):
        ...
        if self.general_service is None:
            raise Exception('General service not initialized')
        print(f'Write config: {MLM2PROUtils.byte_array_to_hex_string(data)}')
        await self.mlm2pro_client.write_characteristic(self.general_service,
            self.encryption.encrypt(data),
            MLM2PROBluetoothAPI.CONFIGURE_CHARACTERISTIC_UUID, True)
    ...
    async def write_command(self, data):
        ...
        await self.mlm2pro_client.write_characteristic(self.general_service,
            self.encryption.encrypt(data),
            MLM2PROBluetoothAPI.COMMAND_CHARACTERISTIC_UUID, True)
