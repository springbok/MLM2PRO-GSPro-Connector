...
async def main(args: argparse.Namespace):
    ...
    await scanner.stop()
    for i in range(0,len(scanner.discovered_devices)):
        #Print the devices discovered
        print("[" + str(i) + "]" + scanner.discovered_devices[i].address,scanner.discovered_devices[i].name,scanner.discovered_devices[i].metadata["uuids"])
